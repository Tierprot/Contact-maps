...
def organize_pair(contacts, chain_A, chain_B):
    result = []
    for i in range(len(contacts)):
        if contacts[i][0][-1] == chain_A and contacts[i][1][-1] == chain_B:
            result.append(contacts[i])
        if contacts[i][0][-1] == chain_B and contacts[i][1][-1] == chain_A:
            result.append(contacts[i])
...
if __name__ == '__main__':
    ...
    inputname = "2M4J_42_3_minimization_"
    inputchainA = "A"
    inputchainB = "B"
    min_distance = 3
    max_distance = 30
    ...
    mol = filter_by_intermolecule(contacts, inputchainA, inputchainB)
    ...
    #организуем пары контактов, так что каждая пара выглядит как (Ai,Bj,contact)
    organize_pair(mol, inputchainA, inputchainB)
    ...
    plt.xlabel('chain {0}'.format(inputchainA))
    plt.ylabel('chain {0}'.format(inputchainB))
